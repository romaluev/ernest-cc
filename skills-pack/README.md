...
| Skill | What it does |
|---|---|
| **account-followup-recovery** | Finds genuinely-dropped follow-ups by searching **all** your tools (mail, HubSpot, Slack, calendar) and **cross-checking** each one for whether it was already handled elsewhere (so it never flags something you already closed). First sweep covers 12 months, then incremental. |
| **add-collaborator** | Flags B2B/partnership threads where the designated teammate isn't included, and adds them — so intros stop falling through the CEO. |
| **candidate-followup** | Finds candidate threads in your inbox, ranks them, and prepares follow-ups for review. |
| **list-sync** | Reconciles an email/segment against a CRM list or a spreadsheet and proposes what's missing on each side — no writes until you approve. |
| **sourcing-pipeline** | Tracks and surfaces sourcing targets that still need outreach; add a profile by URL. |
| **talent-sourcing-grading** | Casts a **wide net**, then ranks candidates by a fit **score** (Tier-1/2/3) so the strongest surface first. Pool + criteria are editable config. |
| **slack-task-tracker** | Pulls open/overdue tasks and asks out of Slack, by owner, so work in threads doesn't get lost. |
...
3. **Just ask**, in plain words:
   ...
   - "Make sure my teammate is on every B2B intro thread."
   - "Find B2B sales/marketing candidates in my inbox and draft follow-ups."
   - "Sync my regional contacts against the team's HubSpot list — what's missing?"
   - "Source more candidates from my target pool and rank them."
